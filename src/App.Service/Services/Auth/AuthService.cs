...
public class AuthService : BaseService, IAuthService
{
    ...
    public string GetToken(AuthLoginOutput user)
    {
        ...
        var claims = new List<Claim>()
       {
            ...
            new Claim(ClaimAttributes.UserType, user.Type.ConvertTo<int>().ToString(), ClaimValueTypes.Integer32),
            new Claim(JwtRegisteredClaimNames.Iat, DateTime.Now.ToMilliseconds().ToString(), ClaimValueTypes.Integer64),
        };
        ...
        if (_appConfig.Value.Value.Tenant)
        {
            claims.AddRange(new[]
            {
                ...
                new Claim(ClaimAttributes.TenantType, user.Tenant?.TenantType.ConvertTo<int>().ToString(), ClaimValueTypes.Integer32),
    ...
    public async Task<AuthGetPasswordEncryptKeyOutput> GetPasswordEncryptKeyAsync()
    {
        ...
        //创建key
        byte[] keyBytes = Encoding.Default.GetBytes(Helper.GenerateRandom(16));
        string keyHexString = BitConverter.ToString(keyBytes);
        var encryptKey = keyHexString.Replace("-", "").ToLower();
        //创建iv
        byte[] ivBytes = Encoding.Default.GetBytes(Helper.GenerateRandom(16));
